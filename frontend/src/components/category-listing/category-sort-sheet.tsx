...
export function CategorySortSheet({
  open,
  value,
  onOpenChange,
  onSelect,
}: CategorySortSheetProps) {
  const titleId = useId();
      ...
      {open ? (
        ...
        >
          ...
          <motion.div
            ...
            className={cn(
              "relative z-[101] flex max-h-[35vh] min-h-[30vh] w-full flex-col",
          ...
          >
            ...
            <div className="flex shrink-0 items-center border-b border-neutral-100 px-5 pb-3 pt-1">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-[max(0.75rem,env(safe-area-inset-bottom))]">
              <CategorySortOptionsList value={value} onSelect={pick} variant="sheet" />
            </div>
          </motion.div>
        </motion.div>
      ) : null}
